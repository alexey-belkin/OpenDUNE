...
WidgetProperties g_widgetProperties[23] = {
	/* x   y   w    h   p4  norm sel */
	...
	{ 1,   6, 12,   3,   0,  15,  6}, /* 21 */
	/* Game controls: the options window plus room for the debug row. */
	{ 5,  42, 30, 150,   0,   0,  0}  /* 22 */
};
...
uint8 GUI_Widget_GetShortcut(uint8 c)
{
	...
	if (c < sizeof(shortcuts)) return shortcuts[c];
	else return 0;
}
