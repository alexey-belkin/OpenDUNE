...
bool GUI_Widget_Mentat_Click(Widget *w)
{
	...
	Music_Play(Tools_RandomUI_Range(0, 5) + 8);
...
void GUI_Mentat_Animation(uint16 speakingMode)
{
	...
	if (movingOtherTimer < g_timerGUI && !g_disableOtherMovement) {
		...
		switch (g_playerHouseID) {
			case HOUSE_HARKONNEN:
				...
				break;
			case HOUSE_ATREIDES:
				movingOtherTimer = g_timerGUI + 60 * Tools_RandomUI_Range(1,3);
				break;
			case HOUSE_ORDOS:
				if (otherSprite != 0) {
					movingOtherTimer = g_timerGUI + 6;
				} else {
					movingOtherTimer = g_timerGUI + 60 * Tools_RandomUI_Range(10, 19);
				}
				break;
			default:
				break;
	...
	if (speakingMode == 1) {
		if (movingMouthTimer < g_timerGUI) {
			...
			movingMouthSprite = Tools_RandomUI_Range(0, 4);
			...
			switch (movingMouthSprite) {
				case 0:
					movingMouthTimer = g_timerGUI + Tools_RandomUI_Range(7, 30);
					break;
				case 1:
				case 2:
				case 3:
					movingMouthTimer = g_timerGUI + Tools_RandomUI_Range(6, 10);
					break;
				case 4:
					movingMouthTimer = g_timerGUI + Tools_RandomUI_Range(5, 6);
					break;
				default:
					break;
			}
		...
		if (Input_Test(0x41) == 0 && Input_Test(0x42) == 0) {
			...
			if (movingMouthTimer != 0xFFFFFFFF) {
				...
				movingMouthSprite = Tools_RandomUI_Range(1, 4);
				partNeedsRedraw = true;
			}
		} else {
			if (movingMouthSprite != 0) {
	...
	if (Mouse_InsideRegion((int16)s_eyesLeft - 16, (int16)s_eyesTop - 8, s_eyesRight + 16, s_eyesBottom + 24) != 0) {
		...
		if (movingEyesNextSprite != 0) {
			...
			if (movingEyesSprite != 4) {
				movingEyesTimer = g_timerGUI + Tools_RandomUI_Range(20, 180);
			} else {
				movingEyesTimer = g_timerGUI + Tools_RandomUI_Range(12, 30);
			}
		} else {
			i = 0;
			switch (speakingMode) {
				case 0:
					i = Tools_RandomUI_Range(0, 7);
					if (i > 5) {
						i = 1;
					} else {
						if (i == 5) {
			...
			if ((i == 2 && movingEyesSprite == 1) || (i == 1 && movingEyesSprite == 2)) {
				...
				movingEyesSprite = 0;
				movingEyesTimer = g_timerGUI + Tools_RandomUI_Range(1, 5);
			} else {
				if (i != movingEyesSprite && (i == 4 || movingEyesSprite == 4)) {
					...
					movingEyesSprite = i;
					if (i != 4) {
						movingEyesTimer = g_timerGUI + Tools_RandomUI_Range(15, 180);
					} else {
						movingEyesTimer = g_timerGUI + Tools_RandomUI_Range(6, 60);
					}
				}
			}
