...
/* SDLK_COLON (0x3A) and SDLK_SEMICOLON (0x3B) are the ';' key, XT scancode
 * 0x27 -- next to L, as the translation table in input.c says.  They were
 * mapped to 0x34 and 0x33, the '.' and ',' keys, so the semicolon key typed a
 * comma and Shift on it typed '<': there was no way to type "host:port" into
 * the lobby's relay row, and a relay on the local network could not be named. */
static const uint8 s_SDL_keymap[] = {
        ...
        0x0B, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x27, 0x27, 0x56, 0x0D,    0,    0, /*  0x30 -  0x3F */
